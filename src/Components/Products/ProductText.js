...
import AddToCardHook from '../../hook/card/add-to-card-hook';
import { ToastContainer } from 'react-toastify';

const ProductText = () => {
  const { id } = useParams();
  const [item, images, cat, brand] = ViewProductsDetalisHook(id);
  const [indexColor,colorText,colorClick,addToCard] = AddToCardHook(id , item);
        ...
        <Col md="8" className="mt-1 d-flex">
          {
            item.availableColors ? (item.availableColors.map((color, index) => {
             ...
             <div className="cat-text d-inline">الكميه المتاحه : {item.quantity}</div>
        </Col>
      </Row>
        ...
        <Col md="12">
                     {
                        item.priceAfterDiscount >= 1 ?(
                          <div className="product-price d-inline px-3 py-3 border">
                                <span style={{textDecorationLine:"line-through"}}>{item.price}</span>
          ...
          <div onClick={addToCard} className="product-cart-add px-3 py-3 d-inline mx-3">اضف للعربة</div>
        </Col>
      </Row>
      <ToastContainer/>
    </div>
